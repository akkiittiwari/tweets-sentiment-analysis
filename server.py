...
def add_color(tweets):
    ...
    for t in tweets:
        score = t['score']['compound']
        scaled_score = int((score+1.0)/(2.0) * (99) + 1)
        t['color'] = colors[scaled_score-1]


@app.route("/<name>")
def tweets(name):
    """Display the tweets for a screen name color-coded by sentiment score"""
    final_dict = fetch_tweets(api, name)
    score_list = list()
    add_color(final_dict['tweets'])
    for tweet in final_dict['tweets']:
        score_list.append(float(tweet['score']['compound']))
    median_score = median(score_list)
    return render_template('tweets.html',
                           user=final_dict['user'],
                           tweets=final_dict['tweets'],
                           median=median_score
                           )
...
def following(name):
    ...
    """
    dict_list = fetch_following(api, name)
